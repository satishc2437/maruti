...
# Deterministic Rules (`rules.json`) (mandatory)

Every generated agent package must include `{artifact-name}-internals/rules.json`.

The generated agent MUST:
- Read `rules.json` at session start.
- Treat the rules as authoritative for filesystem access.
- Enforce `base` as:
   - `"repo"`: patterns are evaluated relative to the repository root containing the agent package.
   - `"package"`: patterns are evaluated relative to the agent package directory.
   - `"workspace"`: patterns are evaluated relative to the VS Code workspace folder root containing the agent package.
      - Deterministic resolution: use the workspace folder that contains the agent package (or `rules.json`), not the active editor.
   - `"absolute"`: patterns are OS-absolute paths.
- Enforce `fs.readOnly` and `fs.readWrite` via allow/deny glob patterns.
- Use deterministic semantics: deny overrides allow; if not explicitly allowed, it is forbidden.
- Gate on `schemaVersion`, and ignore unknown top-level keys for forward compatibility.
...
- Provide exactly three outputs:
   ...
   3. A `ReadMe.md` file documenting the agent's purpose, behavior, and usage instructions. The ReadMe must include:
