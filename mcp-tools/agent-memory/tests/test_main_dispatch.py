"""Tests for agent_memory.__main__ CLI dispatch.
...
from agent_memory.__main__ import main, parse_args
...
def test_main_default_dispatches_to_run_server():
    ...
    with patch("agent_memory.__main__.asyncio.run") as mock_run, \
         patch.object(sys, "argv", ["agent_memory"]):
        main()
...
def test_main_with_test_flag_dispatches_to_self_test():
    ...
    with patch("agent_memory.__main__.asyncio.run") as mock_run, \
         patch.object(sys, "argv", ["agent_memory", "--test"]):
        main()
    mock_run.assert_called_once()
    coro = mock_run.call_args[0][0]
...
def test_main_handles_keyboard_interrupt_without_raising(capsys):
    ...
    with patch(
        "agent_memory.__main__.asyncio.run", side_effect=KeyboardInterrupt(),
    ), patch.object(sys, "argv", ["agent_memory"]):
        main()  # must not raise
...
def test_main_unexpected_exception_exits_with_code_one(capsys):
    ...
    with patch(
        "agent_memory.__main__.asyncio.run", side_effect=RuntimeError("boom"),
    ), patch.object(sys, "argv", ["agent_memory"]):
