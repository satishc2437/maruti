...
uvx --from "git+https://github.com/<owner>/<repo>.git@<ref>#subdirectory=mcp-tools/pdf-reader" \
  python -m pdf_reader
...
Use this as a starting point for an MCP client `mcp.json`/server configuration that
   ...
   ```json
   {
     "mcpServers": {
       "pdf-reader": {
         ...
         "cwd": "/path/to/your/mcp-tools/pdf-reader"
       }
     }
   }
   ```
...
```bash
uvx --from /path/to/mcp-tools/pdf-reader python -m pdf_reader
