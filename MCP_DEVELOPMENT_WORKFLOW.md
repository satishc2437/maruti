...
Docstring checks are enforced via `ruff` pydocstyle rules (`D*`) using the Google
docstring convention. Run this from the repo root:

```bash
uv run ruff check --select D mcp-tools/*/src
```

### Pylint (Errors/Warnings Gate)

Pylint is enforced for errors/warnings (not refactor/convention). Run this from
the repo root (it uses the root `.pylintrc`):
